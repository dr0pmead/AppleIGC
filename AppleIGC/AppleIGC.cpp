...
enum { CSUM_TCPIPv6 = 0x0020, CSUM_UDPIPv6 = 0x0040 };
...
static inline struct tcphdr *tcp6_hdr(mbuf_t skb) {
  ...
  if (next_header != IPPROTO_TCP || offset + sizeof(struct tcphdr) > pkt_len) {
    return NULL;
  }
...
static void kfree(void *p, size_t size) { IOFree(p, size); }
...
static void vfree(void *p, size_t size) { IOFreeAligned(p, size); }
...
static int netif_queue_stopped(IOEthernetController *netdev) {
  return ((AppleIGC *)netdev)->queueStopped() ||
         ((AppleIGC *)netdev)->Stalled();
}
#endif
...
static mbuf_t netdev_alloc_skb_ip_align(IOEthernetController *netdev,
                                        u16 rx_buffer_len) {
...
static const char igc_copyright[] = "Copyright(c) 2018 Intel Corporation.";
...
void igc_reset(struct igc_adapter *adapter) {
  ...
  /* Re-establish EEE setting - Force DISABLED for stability */
...
static void igc_release_hw_control(struct igc_adapter *adapter) {
  ...
  wr32(IGC_CTRL_EXT, ctrl_ext & ~IGC_CTRL_EXT_DRV_LOAD);
}
...
#ifndef __APPLE__
  dma_unmap_single(dev, dma_unmap_addr(buf, dma), dma_unmap_len(buf, len),
                   DMA_TO_DEVICE);
#endif
...
#ifdef __APPLE__
  tx_ring->pool = IOBufferMemoryDescriptor::inTaskWithOptions(
...
#else
    dma_sync_single_range_for_cpu(rx_ring->dev, buffer_info->dma,
                                  buffer_info->page_offset,
                                  igc_rx_bufsz(rx_ring), DMA_FROM_DEVICE);

    /* free resources associated with mapping */
    dma_unmap_page_attrs(rx_ring->dev, buffer_info->dma,
...
#ifdef __APPLE__
  rx_ring->pool = IOBufferMemoryDescriptor::inTaskWithOptions(
      kernel_task, kIODirectionInOut | kIOMemoryPhysicallyContiguous,
...
static void igc_configure_rx_ring(struct igc_adapter *adapter,
  ...
  wr32(IGC_RDBAL(reg_idx), rdba & 0x00000000ffffffffULL);
  wr32(IGC_RDBAH(reg_idx), rdba >> 32);
  wr32(IGC_RDLEN(reg_idx), ring->count * sizeof(union igc_adv_rx_desc));
  ...
  memset(ring->rx_buffer_info, 0, sizeof(struct igc_rx_buffer) * ring->count);
...
static void igc_configure_tx_ring(struct igc_adapter *adapter,
  ...
  wr32(IGC_TDBAL(reg_idx), tdba & 0x00000000ffffffffULL);
...
static void igc_setup_tctl(struct igc_adapter *adapter) {
  ...
  tctl &= ~IGC_TCTL_CT;
  tctl |=
      IGC_TCTL_PSP | IGC_TCTL_RTLC | (IGC_COLLISION_THRESHOLD << IGC_CT_SHIFT);
...
static void igc_tx_ctxtdesc(struct igc_ring *tx_ring, __le32 launch_time,
                            bool first_flag, u32 vlan_macip_lens,
...
#define DEMAND_IPv4                                                            \
  (IONetworkController::kChecksumIP | IONetworkController::kChecksumTCP |      \
   IONetworkController::kChecksumUDP)
#define DEMAND_MASK (DEMAND_IPv6 | DEMAND_IPv4)

  UInt32 checksumDemanded;
  tx_ring->netdev->getChecksumDemand(
      skb, IONetworkController::kChecksumFamilyTCPIP, &checksumDemanded);
  ...
  igc_tx_ctxtdesc(tx_ring, launch_time, first_flag, vlan_macip_lens, type_tucmd,
                  mss_l4len_idx);
}
...
static u32 igc_tx_cmd_type(struct sk_buff *skb, u32 tx_flags) {
  ...
  cmd_type |= IGC_SET_FLAG(tx_flags, IGC_TX_FLAGS_VLAN, IGC_ADVTXD_DCMD_VLE);

  /* set segmentation bits for TSO */
  cmd_type |= IGC_SET_FLAG(tx_flags, IGC_TX_FLAGS_TSO, (IGC_ADVTXD_DCMD_TSE));

  /* set timestamp bit if present */
  cmd_type |=
      IGC_SET_FLAG(tx_flags, IGC_TX_FLAGS_TSTAMP, (IGC_ADVTXD_MAC_TSTAMP));
...
static void igc_tx_olinfo_status(struct igc_ring *tx_ring,
                                 union igc_adv_tx_desc *tx_desc, u32 tx_flags,
                                 unsigned int paylen) {
  u32 olinfo_status = paylen << IGC_ADVTXD_PAYLEN_SHIFT;

  /* insert L4 checksum */
  olinfo_status |= (tx_flags & IGC_TX_FLAGS_CSUM) *
                   ((IGC_TXD_POPTS_TXSM << 8) / IGC_TX_FLAGS_CSUM);

  /* insert IPv4 checksum */
  olinfo_status |= (tx_flags & IGC_TX_FLAGS_IPV4) *
                   (((IGC_TXD_POPTS_IXSM << 8)) / IGC_TX_FLAGS_IPV4);

  tx_desc->read.olinfo_status = cpu_to_le32(olinfo_status);
}

static int igc_tx_map(struct igc_ring *tx_ring, struct igc_tx_buffer *first,
                      const u8 hdr_len, struct IOPhysicalSegment *vec,
                      UInt32 frags) {
  struct sk_buff *skb = first->skb;
  struct igc_tx_buffer *tx_buffer;
  ...
  for (k = 0; k < frags; k++) {
    count +=
        (vec[k].length + (IGC_MAX_DATA_PER_TXD - 1)) / IGC_MAX_DATA_PER_TXD;
  }
  ...
  igc_tx_olinfo_status(tx_ring, tx_desc, tx_flags,
                       mbuf_pkthdr_len(skb) - hdr_len);
...
#endif
  {
    ...
    while (unlikely(size > IGC_MAX_DATA_PER_TXD)) {
      tx_desc->read.cmd_type_len = cpu_to_le32(cmd_type ^ IGC_MAX_DATA_PER_TXD);
...
static inline u_short in_pseudo(u_int a, u_int b, u_int c) {
  u_int64_t sum;
  union q_util q_util;
  union l_util l_util;
...
static inline u_short in_pseudo6(struct ip6_hdr *ip6, int nxt, u_int32_t len) {
  u_int16_t *w;
  int sum = 0;
  union {
    u_int16_t phs[4];
...
static int igc_tso(struct igc_ring *tx_ring, struct igc_tx_buffer *first,
                   __le32 launch_time, bool first_flag, u8 *hdr_len) {
  ...
  if (request & MBUF_TSO_IPV4) {
    ...
    iph->ip_sum = 0;
    tcph->th_sum =
        in_pseudo(iph->ip_src.s_addr, iph->ip_dst.s_addr, htonl(IPPROTO_TCP));

    type_tucmd |= IGC_ADVTXD_TUCMD_IPV4;
    first->tx_flags |= IGC_TX_FLAGS_TSO | IGC_TX_FLAGS_CSUM | IGC_TX_FLAGS_IPV4;
  } else {
    ...
    first->tx_flags |= IGC_TX_FLAGS_TSO | IGC_TX_FLAGS_CSUM;
  }
...
static void igc_rx_checksum(struct igc_ring *ring,
  ...
  if (igc_test_staterr(rx_desc,
                       IGC_RXDEXT_STATERR_L4E | IGC_RXDEXT_STATERR_IPE)) {
  ...
  if (igc_test_staterr(rx_desc, IGC_RXD_STAT_TCPCS | IGC_RXD_STAT_UDPCS))
...
static void igc_rx_vlan(struct igc_ring *rx_ring,
                        union igc_adv_rx_desc *rx_desc, struct sk_buff *skb) {
...
#ifdef HAVE_VLAN_RX_REGISTER
static void igc_vlan_mode(IOEthernetController *netdev,
...
static struct igc_rx_buffer *igc_get_rx_buffer(struct igc_ring *rx_ring,
  ...
  *rx_buffer_pgcnt =
  ...
  dma_sync_single_range_for_cpu(rx_ring->dev, rx_buffer->dma,
                                rx_buffer->page_offset, size, DMA_FROM_DEVICE);
 ...
 */
static int igc_add_rx_frag(struct igc_ring *rx_ring,
                           struct igc_rx_buffer *rx_buffer, struct sk_buff *skb,
                           unsigned int size) {
#ifdef __APPLE__
  ...
  if (unlikely(mbuf_copyback(skb, orig_len, size, va, MBUF_WAITOK))) {
    pr_err("Unexpected mbuf_copyback()\n");
    return -1;
  }

  // Verify mbuf integrity after copyback - mbuf_copyback can create
  // additional mbufs in the chain, so we need to ensure M_PKTHDR is still set
  if (unlikely(!(mbuf_flags(skb) & M_PKTHDR))) {
    pr_err("CRITICAL: mbuf lost M_PKTHDR flag after copyback, flags=0x%x\n",
           (unsigned)mbuf_flags(skb));
    // This is a critical error - the mbuf is now corrupted
    // The caller should handle this by dropping the packet
    return -1;
  }
  return 0;
#else
...
#else
  truesize = ring_uses_build_skb(rx_ring) ? SKB_DATA_ALIGN(IGC_SKB_PAD + size)
                                          : SKB_DATA_ALIGN(size);
#endif
...
static bool igc_can_reuse_rx_page(struct igc_rx_buffer *rx_buffer,
  ...
  if (unlikely((rx_buffer_pgcnt - pagecnt_bias) > 1))
    return false;
#else
#define IGC_LAST_OFFSET (SKB_WITH_OVERHEAD(PAGE_SIZE) - IGC_RXBUFFER_2048)
...
static bool igc_alloc_mapped_page(struct igc_ring *rx_ring,
  ...
  bi->page = IOBufferMemoryDescriptor::inTaskWithOptions(
      kernel_task, kIODirectionInOut | kIOMemoryPhysicallyContiguous, PAGE_SIZE,
      PAGE_SIZE);

  if (unlikely(!bi->page)) {
    pr_err(
        "Failed to alloc new page for storage from IOBufferMemoryDescriptor\n");
  ...
  /* map page for use */
  dma = dma_map_page_attrs(rx_ring->dev, page, 0, igc_rx_pg_size(rx_ring),
...
//    IOBufferMemoryDescriptor *page;
  ...
  if (likely(!skb)) {
    ...
    /* allocate a skb to store the frags */
    skb = netdev_alloc_skb_ip_align(rx_ring->netdev, IGC_RX_HDR_LEN);
    if (unlikely(!skb)) {
      rx_ring->rx_stats.alloc_failed++;
      return NULL;
    }
  ...
  /* pull page into skb */
  if (igc_add_rx_frag(rx_ring, rx_buffer, skb, size) != 0) {
...
static int igc_clean_rx_irq(struct igc_q_vector *q_vector, const int budget) {
  ...
  while (likely(total_packets < budget)) {
    ...
    // Defensive check: Ensure skb has M_PKTHDR before touching length
    if (!(mbuf_flags(skb) & M_PKTHDR)) {
      pr_err(
          "AppleIGC: RX Panic Prevented: skb missing M_PKTHDR in IRQ loop\n");
      netdev->freePacket(skb);
      skb = NULL;
      continue;
    }
...
static bool igc_clean_tx_irq(struct igc_q_vector *q_vector, int napi_budget) {
  ...
  if (test_bit(IGC_RING_FLAG_TX_DETECT_HANG, &tx_ring->flags)) {
    ...
    if (tx_buffer->next_to_watch &&
        time_after(jiffies,
                   tx_buffer->time_stamp + (adapter->tx_timeout_factor * HZ)) &&
      ...
      netdev_err(tx_ring->netdev,
                 ...
                 "  jiffies              <%llx>\n"
                 "  desc.status          <%x>\n",
                 tx_ring->queue_index, rd32(IGC_TDH(tx_ring->reg_idx)),
                 readl(tx_ring->tail), tx_ring->next_to_use,
                 tx_ring->next_to_clean, tx_buffer->time_stamp,
                 tx_buffer->next_to_watch, jiffies,
...
static int igc_find_mac_filter(struct igc_adapter *adapter,
  ...
  for (i = 0; i < max_entries; i++) {
    ...
    if (!!(rah & IGC_RAH_ASEL_SRC_ADDR) != type)
      continue;
    if ((rah & IGC_RAH_RAH_MASK) != le16_to_cpup((__le16 *)(addr + 4)))
      continue;
...
static int igc_add_mac_filter(struct igc_adapter *adapter,
  ...
  netdev_dbg(
      ...
      index, type == IGC_MAC_FILTER_TYPE_DST ? "dst" : "src", addr, queue);
...
static void igc_del_mac_filter(struct igc_adapter *adapter,
  ...
  if (index == 0) {
    ...
    netdev_dbg(dev, "Delete MAC address filter: index %d type %s address %pM\n",
               index, type == IGC_MAC_FILTER_TYPE_DST ? "dst" : "src", addr);
...
static int igc_flex_filter_select(struct igc_adapter *adapter,
                                  struct igc_flex_filter *input, u32 *fhft) {
  struct igc_hw *hw = &adapter->hw;
  u8 fhft_index;
  u32 fhftsl;
  ...
  *fhft =
      (fhft_index < 4) ? IGC_FHFT(fhft_index) : IGC_FHFT_EXT(fhft_index - 4);
...
static int igc_write_flex_filter_ll(struct igc_adapter *adapter,
  ...
  for (i = 0; i < 16; ++i) {
    ...
    u32 dw0 = (data[data_idx + 0] << 0) | (data[data_idx + 1] << 8) |
              (data[data_idx + 2] << 16) | (data[data_idx + 3] << 24);
    u32 dw1 = (data[data_idx + 4] << 0) | (data[data_idx + 5] << 8) |
              (data[data_idx + 6] << 16) | (data[data_idx + 7] << 24);
    u32 tmp;
...
static int igc_add_flex_filter(struct igc_adapter *adapter,
  ...
  if (rule->filter.match_flags & IGC_FILTER_FLAG_DST_MAC_ADDR)
    igc_flex_filter_add_field(&flex, &filter->dst_addr, 0, ETH_ALEN, NULL);

  /* Add source MAC */
  if (rule->filter.match_flags & IGC_FILTER_FLAG_SRC_MAC_ADDR)
    igc_flex_filter_add_field(&flex, &filter->src_addr, 6, ETH_ALEN, NULL);

  /* Add VLAN etype */
  if (rule->filter.match_flags & IGC_FILTER_FLAG_VLAN_ETYPE)
    igc_flex_filter_add_field(&flex, &filter->vlan_etype, 12,
                              sizeof(filter->vlan_etype), NULL);
  ...
  if (rule->filter.match_flags & IGC_FILTER_FLAG_ETHER_TYPE) {
    ...
    igc_flex_filter_add_field(&flex, &etype, eth_offset, sizeof(etype), NULL);
  }

  /* Add user data */
  if (rule->filter.match_flags & IGC_FILTER_FLAG_USER_DATA)
    igc_flex_filter_add_field(&flex, &filter->user_data, user_offset,
                              sizeof(filter->user_data), filter->user_mask);
...
static void igc_del_flex_filter(struct igc_adapter *adapter, u16 reg_index) {
  struct igc_hw *hw = &adapter->hw;
  u32 wufc;
...
static int igc_uc_sync(IOEthernetController *netdev,
...
static int igc_uc_unsync(IOEthernetController *netdev,
                         const unsigned char *addr) {
  struct igc_adapter *adapter = netdev_priv(netdev);
 ...
 */
static void igc_write_ivar(struct igc_hw *hw, int msix_vector, int index,
                           int offset) {
...
static void igc_assign_vector(struct igc_q_vector *q_vector, int msix_vector) {
  ...
  case igc_i225:
    if (rx_queue > IGC_N0_QUEUE)
      igc_write_ivar(hw, msix_vector, rx_queue >> 1, (rx_queue & 0x1) << 4);
    if (tx_queue > IGC_N0_QUEUE)
      igc_write_ivar(hw, msix_vector, tx_queue >> 1,
                     ((tx_queue & 0x1) << 4) + 8);
...
static void igc_configure_msix(struct igc_adapter *adapter) {
  ...
  case igc_i225:
     ...
     */
    wr32(IGC_GPIE,
         IGC_GPIE_MSIX_MODE | IGC_GPIE_PBA | IGC_GPIE_EIAME | IGC_GPIE_NSICR);
...
static void igc_set_interrupt_capability(struct igc_adapter *adapter,
  ...
  // adapter->msix_entries = (struct msix_entry *)kcalloc(numvecs, sizeof(struct
  // msix_entry));
  ...
  err = pci_enable_msix_range(adapter->pdev, adapter->msix_entries, numvecs,
                              numvecs);
  if (err > 0)
    return;
...
static void igc_update_ring_itr(struct igc_q_vector *q_vector) {
  ...
  if (packets)
    avg_wire_size =
        max_t(u32, avg_wire_size, q_vector->tx.total_bytes / packets);
 ...
 */
static int igc_alloc_q_vector(struct igc_adapter *adapter, unsigned int v_count,
                              unsigned int v_idx, unsigned int txr_count,
                              unsigned int txr_idx, unsigned int rxr_count,
  ...
  if (!q_vector) {
    ...
    //  GFP_KERNEL);
    q_vector = (struct igc_q_vector *)kzalloc(
        sizeof(struct igc_q_vector) + (sizeof(struct igc_ring) * ring_count));
  } else {
    // memset(q_vector, 0, struct_size(q_vector, ring, ring_count));
    memset(q_vector, 0,
           sizeof(struct igc_q_vector) +
...
static int igc_alloc_q_vectors(struct igc_adapter *adapter) {
  ...
  if (q_vectors >= (rxr_remaining + txr_remaining)) {
    for (; rxr_remaining; v_idx++) {
      err = igc_alloc_q_vector(adapter, q_vectors, v_idx, 0, 0, 1, rxr_idx);
  ...
  for (; v_idx < q_vectors; v_idx++) {
    ...
    err = igc_alloc_q_vector(adapter, q_vectors, v_idx, tqpv, txr_idx, rqpv,
                             rxr_idx);
...
static int igc_sw_init(struct igc_adapter *adapter) {
  ...
  adapter->max_frame_size = netdev->mtu() + ETH_HLEN + ETH_FCS_LEN + VLAN_HLEN;
...
void igc_update_stats(struct igc_adapter *adapter) {
  ...
  net_stats->inputErrors =
      (u32)(adapter->stats.rxerrc + adapter->stats.crcerrs +
  ...
  // net_stats->outputErrors = (u32)(adapter->stats.ecol +
  // adapter->stats.latecol);
...
static int igc_set_features(AppleIGC *netdev, netdev_features_t features) {
...
static netdev_features_t igc_features_check(struct sk_buff *skb,
  ...
  if (unlikely(network_hdr_len > IGC_MAX_NETWORK_HDR_LEN))
    return features &
           ~(NETIF_F_HW_CSUM | NETIF_F_SCTP_CRC | NETIF_F_TSO | NETIF_F_TSO6);
...
static int igc_request_msix(struct igc_adapter *adapter) {
  ...
  err = request_irq(adapter->msix_entries[vector].vector, &igc_msix_other, 0,
                    netdev->name, adapter);
  if (err)
    goto err_out;
  ...
  for (i = 0; i < num_q_vectors; i++) {
    ...
    err = request_irq(adapter->msix_entries[vector].vector, igc_msix_ring, 0,
...
err_free:
  ...
  vector--;
  for (i = 0; i < vector; i++) {
    free_irq(adapter->msix_entries[free_vector++].vector, adapter->q_vector[i]);
  }
err_out:
  return err;
#endif
...
static int igc_request_irq(struct igc_adapter *adapter) {
  ...
  if (adapter->flags & IGC_FLAG_HAS_MSI) {
    err = request_irq(pdev->irq, &igc_intr_msi, 0, netdev->name, adapter);
    if (!err)
      goto request_done;
  ...
  err = request_irq(pdev->irq, &igc_intr, IRQF_SHARED, netdev->name, adapter);
#endif
  if (err)
...
int igc_open(IOEthernetController *netdev) { return __igc_open(netdev, false); }
...
s32 igc_read_pcie_cap_reg(struct igc_hw *hw, u32 reg, u16 *value) {
  ...
  if (0 ==
      adapter->pdev->findPCICapability(kIOPCIPCIExpressCapability, &cap_offset))
...
static const struct {
  ...
  const char *name;
    ...
    {IGC_DEV_ID_I226_V, "I226-V"},   {IGC_DEV_ID_I226_LM, "I226-LM"},
    {IGC_DEV_ID_I226_IT, "I226-IT"}, {IGC_DEV_ID_I226_K, "I226-K"},
    {IGC_DEV_ID_I225_V, "I225-V"},   {IGC_DEV_ID_I225_LM, "I225-LM"},
    {IGC_DEV_ID_I225_I, "I225-I"},   {IGC_DEV_ID_I225_IT, "I225-IT"},
};
...
bool AppleIGC::igc_probe() {
  ...
  }
  {
    ...
    reg16 |= (kIOPCICommandBusMaster | kIOPCICommandMemorySpace);
  ...
  _features |= NETIF_F_SG | NETIF_F_IP_CSUM |
...
int AppleIGC::getIntOption(const char *name, int defVal, int maxVal,
                           int minVal) {
  int val = defVal;
...
#ifdef NETIF_F_TSO
  useTSO = FALSE; // getBoolOption("NETIF_F_TSO", TRUE);
#else
  useTSO = FALSE;
#endif
...
void AppleIGC::setLinkUp() {
  ...
  hw->mac.ops.get_speed_and_duplex(hw, &adapter->link_speed,
                                   &adapter->link_duplex);
  ...
  pr_debug("[LU]: Link Up on en%u (%s), %s, %s, %s%s (igc driver ver %08x)\n",
           netif->getUnitNumber(), deviceModelNames[chip_idx].name, speedName,
           duplexName, flowName, eeeName, 0);
  pr_debug("setLinkUp() <===\n");
}
...
void AppleIGC::systemWillShutdown(IOOptionBits specifier) {
  ...
  if ((kIOMessageSystemWillPowerOff | kIOMessageSystemWillRestart) &
    ...
    priv_adapter.hw.mac.ops.rar_set(&priv_adapter.hw,
                                    priv_adapter.hw.mac.perm_addr, 0);
...
/** This method doesn't completely shutdown NIC. It intentionally keeps
 * eventSources and enables interruptes back
 */
...
IOReturn AppleIGC::outputStart(IONetworkInterface *interface,
                               IOOptionBits options) {
  struct igc_adapter *adapter = &priv_adapter;
  struct igc_ring *tx_ring = igc_tx_queue_mapping(adapter, skb);
  mbuf_t skb = NULL;
  while ((txNumFreeDesc = igc_desc_unused(tx_ring)) >= (MAX_SKB_FRAGS + 3) &&
         kIOReturnSuccess ==
             interface->dequeueOutputPackets(1, &skb, NULL, NULL, NULL)) {
    if (!(mbuf_flags(skb) & M_PKTHDR)) {
    ...
    UInt32 frags =
        txMbufCursor->getPhysicalSegmentsWithCoalesce(skb, vec, MAX_SKB_FRAGS);
    if (frags == 0) {
...
UInt32 AppleIGC::outputPacket(mbuf_t skb, void *param) {
  ...
  do {
    ...
    struct IOPhysicalSegment vec[MAX_SKB_FRAGS];
    UInt32 frags = tx_ring->netdev->txCursor()->getPhysicalSegmentsWithCoalesce(
        skb, vec, MAX_SKB_FRAGS);
    if (frags == 0) {
      pr_debug("No frags by getPhysicalSegmentsWithCoalesce()\n");
      goto error;
    }
    ...
    if (txNumFreeDesc < count + 2 + 3 // igc_maybe_stop_tx
        || stalled) /** even if we have free desc we should exit in stalled mode
                       as queue is enabled by threadsafe interrupts -> native
                       igc code (see igb_poll) */
    {
...
void AppleIGC::getPacketBufferConstraints(
...
UInt16 AppleIGC::intelSupportsEEE(struct igc_adapter *adapter) {
  ...
  if (!hw->dev_spec._base.eee_enable)
    goto done;
  result |=
      IGC_IPCNFG_EEE_2_5G_AN | IGC_IPCNFG_EEE_1G_AN | IGC_IPCNFG_EEE_100M_AN;
...
case MEDIUM_INDEX_2500FDFCEEE:
  ...
  case MEDIUM_INDEX_AUTO:
    ...
    if (intelSupportsEEE(adapter))
      hw->dev_spec._base.eee_enable = false; // true;
...
bool AppleIGC::createWorkLoop() {
  ...
  if (OSCompareAndSwap(0, 1, (UInt32 *)&workLoop)) {
    ...
    while ((IOWorkLoop *)workLoop == (IOWorkLoop *)1)
...
IOWorkLoop *AppleIGC::getWorkLoop() const { return workLoop; }
...
IOReturn AppleIGC::getChecksumSupport(UInt32 *checksumMask,
                                      UInt32 checksumFamily, bool isOutput) {
  *checksumMask = 0;
  if (checksumFamily != kChecksumFamilyTCPIP) {
    pr_err("AppleIGC: Operating system wants information for unknown checksum "
           "family.\n");
    return kIOReturnUnsupported;
  }
  if (!isOutput) {
    *checksumMask =
        kChecksumTCP | kChecksumUDP | kChecksumIP | CSUM_TCPIPv6 | CSUM_UDPIPv6;
  } else {
#if USE_HW_UDPCSUM
...
/**
 * intelCheckLink
 * It's not exact copy of igc_has_link, additional check for E1000_STATUS_LU
 * (Link up) is performed Reference: igc_has_link
 */
...
void AppleIGC::checkLinkStatus() {
  ...
  if (linkUp) {
    if (link) {
       ...
       * of the link parameters to make sure that the MAC is programmed
       * correctly.
       */
      watchdogSource->cancelTimeout();
      igc_update_stats(&priv_adapter);
...
void AppleIGC::interruptHandler(OSObject *target, IOInterruptEventSource *src,
                                int count) {
...
void AppleIGC::watchdogTask() {
  ...
  if (!test_bit(__IGC_DOWN, &adapter->state)) {
    if (adapter->flags & IGC_FLAG_NEED_LINK_UPDATE) {
      pr_debug("watchdogTask(): adapter has IGB_FLAG_NEED_LINK_UPDATE, forcing "
               "restart.\n");
      intelRestart();
    }
  }
...
IOReturn AppleIGC::registerWithPolicyMaker(IOService *policyMaker) {
  static IOPMPowerState powerStateArray[2] = {
      ...
      {1, kIOPMDeviceUsable, kIOPMPowerOn, kIOPMPowerOn, 0, 0, 0, 0, 0, 0, 0,
       0}};
  powerState = 1;
...
IOReturn AppleIGC::setPowerState(unsigned long powerStateOrdinal,
  ...
  if (powerStateOrdinal == 0) { // SUSPEND/SHUTDOWN
    pr_err("suspend start: stopping interface\n");
    if (netif->getInterfaceState() & kIONetworkInterfaceOpenedState) {
      igc_down(&priv_adapter);
    }
    pr_err("suspend end.\n");
    bSuspended = TRUE;
  } else if (bSuspended) { // WAKE
    pr_err("resume start: restarting interface\n");
    if (netif->getInterfaceState() & kIONetworkInterfaceOpenedState) {
      igc_up(&priv_adapter);
    }
    pr_err("resume end.\n");
...
IOReturn AppleIGC::getPacketFilters(const OSSymbol *group,
...
void AppleIGC::startTxQueue() {
  ...
  if (likely(stalled && txMbufCursor && transmitQueue)) {
    ...
    txMbufCursor = IOMbufNaturalMemoryCursor::withSpecification(
        _mtu + ETH_HLEN + ETH_FCS_LEN + VLAN_HLEN, MAX_SKB_FRAGS);
  ...
  if (txMbufCursor == NULL) {
    txMbufCursor =
        IOMbufNaturalMemoryCursor::withSpecification(0x4000, MAX_SKB_FRAGS);
  }
...
bool AppleIGC::carrier() { return (preLinkStatus & kIONetworkLinkActive) != 0; }
...
void AppleIGC::setCarrier(bool stat) {
  ...
  if (stat) {
    ...
    if (!setLinkStatus(preLinkStatus, mediumTable[index])) {
      pr_err("setLinkStatus: Some properties were not updated successullly "
             "with current medium(%u)\n",
             preLinkStatus);
    }
  } else {
    preLinkStatus = kIONetworkLinkValid;
    if (!setLinkStatus(preLinkStatus)) {
      pr_err("setLinkStatus(kIONetworkLinkValid): Some properties were not "
             "updated\n");
    }
  }
...
void AppleIGC::receive(mbuf_t skb) {
  ...
  if (!(mbuf_flags(skb) & M_PKTHDR)) {
    pr_err("AppleIGC: receive() dropped corrupted packet (missing M_PKTHDR)\n");
    this->freePacket(skb);
    return;
  }

  // 2. Clone the packet (Deep safety)
  // We duplicate the mbuf header to ensure the structure passed to the kernel
  // is fresh and isolated from the driver's RX ring memory/flag corruption.
  mbuf_t new_skb = NULL;
  errno_t err = mbuf_dup(skb, MBUF_DONTWAIT, &new_skb);

  if (err != 0 || !new_skb) {
    // Duplication failed (OOM?), drop original
    this->freePacket(skb);
    return;
  }

  // 3. Free the original packet immediately (Driver releases ownership)
  this->freePacket(skb);

  // 4. Validate the clone (Paranoia)
  if (mbuf_pkthdr_len(new_skb) == 0) {
    // pr_err("AppleIGC: receive() dropped zero-length clone\n");
    mbuf_freem(new_skb);
    return;
  }

  // 5. Pass the CLONE to the kernel
  netif->inputPacket(new_skb, (UInt32)mbuf_pkthdr_len(new_skb),
                     IONetworkInterface::kInputOptionQueuePacket);
}

void AppleIGC::flushInputQueue() { netif->flushInputQueue(); }

void AppleIGC::setVid(mbuf_t skb, UInt16 vid) { setVlanTag(skb, vid); }
...
bool AppleIGC::initEventSources(IOService *provider) {
  ...
  interruptSource = IOInterruptEventSource::interruptEventSource(
      this, &AppleIGC::interruptHandler, provider);
  if (!interruptSource) {
  ...
  watchdogSource =
      IOTimerEventSource::timerEventSource(this, &AppleIGC::watchdogHandler);
  if (!watchdogSource) {
  ...
  resetSource =
      IOTimerEventSource::timerEventSource(this, &AppleIGC::resetHandler);
...
#ifdef __PRIVATE_SPI__
  IOReturn error = interface->configureOutputPullModel(
      IGC_MIN_TXD, 0, 0,
...
#endif
  snprintf(modelName, kNameLength, "Intel(R) Ethernet Controller %s (IGC)",
           deviceModelNames[chip_idx].name);
